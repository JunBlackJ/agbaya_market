...
class CategoryDelet(DeleteView):
    ...
    def get_success_url(self):
        return reverse('shop:category_list')
#####
class PartenaireList(ListView):
    model = Partenaire
    ordering = ['-id']


def PartenaireDetail(request, slug):
    try:
        partenaire = Partenaire.objects.get(slug=slug)
    except Partenaire.DoesNotExist:
        raise Http404
    product = Product.objects.filter(partenaire=partenaire)
    return render(request, 'shop/partenaire_detail.html', {'partenaire':partenaire, 'products':product})


class PartenaireCreate(LoginRequiredMixin, CreateView):
    model = Partenaire
    fields = ['name', 'image']
...
class PartenaireUpdate(LoginRequiredMixin, UpdateView):
    model = Partenaire
    fields = ['name', 'image']
...
class PartenaireDelet(DeleteView):
    model = Partenaire
