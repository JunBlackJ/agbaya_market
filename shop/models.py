...
@receiver(models.signals.post_delete, sender=Partenaire)
def auto_delete_file_on_delete(sender, instance, **kwargs):
    """ DELETE PARTENAIRE IMAGE AUTOMATICALLY FROM MEDIA AFTER DELETEING PATENAIRE """
    if instance.image:
        if os.path.isfile(instance.image.path):
            os.remove(instance.image.path)


@receiver(models.signals.pre_save, sender=Partenaire)
def auto_delete_file_on_change(sender, instance, **kwargs):
    """ DELETE PARTENAIRE IMAGE AUTOMATICALLY FROM MEDIA AFTER UPDATING PARTENAIRE """
    if not instance.pk:
        return False
    try:
        old_file = Partenaire.objects.get(pk=instance.pk).image
...
class Product(models.Model):
    ...
    partenaire = models.ForeignKey(Partenaire, on_delete=models.SET_NULL, related_name='product_partenaire',null=True, blank=True)#
